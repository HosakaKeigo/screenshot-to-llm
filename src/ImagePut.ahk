...
ImagePutDesktop(image, title := "", pos := "", style := 0x50000000, styleEx := 0x80000, parent := "", playback := True,
    cache := False) {
...
ImageShow(image, title := "", pos := "", style := 0x90000000, styleEx := 0x80088, parent := "", playback := True, cache :=
    False) {
...
stream:
        ...
        weight |=
            ...
            !(cotype ~= "^(?i:safearray|encodedbuffer|hex|base64|uri|stream|randomaccessstream|)$"
                ...
                || cotype = "file"
                && (!p.Has(1) || p[1] == "" || p[1] ~= "(^|:|\\|\.)" extension "$" && !(extension = "jpg" && p.Has(2) &&
                p[2] != "")
...
bitmap:
    ...
    static ImageType(image) {
        ...
        if image.HasProp("prototype") && image.prototype.HasProp("__class") && image.prototype.__class ==
        "ClipboardAll"
...
array:
        ...
        && image[1] > -65536 && image[1] < 65536 && image[2] > -65536 && image[2] < 65536 && image[3] < 65536 && image[
            4] < 65536
...
string:
        ...
        if DllCall("CloseHandle", "ptr", DllCall("OpenFileMapping", "uint", 2, "int", 0, "str", "ImagePut_" image,
            "ptr"))
            return "SharedBuffer"
        ...
        if not (image ~= "^-?\d+$") {
            for extension in ["bmp", "dib", "rle", "jpg", "jpeg", "jpe", "jfif", "gif", "tif", "tiff", "png", "ico",
                "exe", "dll"] {
...
pointer:
    ...
    static BitmapScale(&pBitmap, scale, direction := 0, bound := "", preserveAspectRatio := False, outDimensions := "") {
        ...
        if Type(scale) == "Array" && scale.length = 2 && (scale.Has(1) && scale[1] ~= "^(?!0+$)\d+$" || scale.Has(2) &&
        scale[2] ~= "^(?!0+$)\d+$") {
        ...
        DllCall("gdiplus\GdipCreateBitmapFromScan0", "int", safe_w, "int", safe_h, "int", 0, "int", format, "ptr", 0,
            "ptr*", &pBitmapScale := 0)
    ...
    static BitmapSprite(&pBitmap) {
        ...
        if !code {
            ...
            DllCall("crypt32\CryptStringToBinary", "str", b64, "uint", 0, "uint", 0x1, "ptr", code, "uint*", n64, "ptr",
                0, "ptr", 0)
                                    ...
                                    : str ~= "(?i)^ff d8 ff" ? "jpg"
                                        : str ~= "(?i)^25 50 44 46 2d" ? "pdf"  ; %PDF-
                                            : str ~= "(?i)^89 50 4e 47 0d 0a 1a 0a" ? "png"  ; PNG
                                                : str ~=
                                                "(?i)^(((?!3c|3e).. )|3c (3f|21) ((?!3c|3e).. )*3e )*+3c 73 76 67" ?
                                                    "svg"  ; <svg
    ...
    static IsImage(bin, size) {
        ...
        DllCall("crypt32\CryptBinaryToString", "ptr", bin, "uint", size, "uint", 0x40000004, "str", str, "uint*", &
            length)
    ...
    static ScreenshotToBuffer(image) {
        ...
        hbm := DllCall("CreateDIBSection", "ptr", hdc, "ptr", bi, "uint", 0, "ptr*", &pBits := 0, "ptr", 0, "uint", 0,
            "ptr")
    ...
    static DesktopDuplicationToBuffer(image) {
        ...
        DllCall("ole32\IIDFromString", "wstr", "{770aae78-f26f-4dba-a829-253c83d1b387}", "ptr", IID_IDXGIFactory1 :=
            Buffer(16), "hresult")
        ...
        while (0x887A0002 != ComCall(EnumAdapters := 7, IDXGIFactory1, "uint", A_Index - 1, "ptr*", &IDXGIAdapter := 0,
            "uint")) {
            while (0x887A0002 != ComCall(EnumOutputs := 7, IDXGIAdapter, "uint", A_Index - 1, "ptr*", &IDXGIOutput := 0,
                "uint")) {
...
AdapterFound:
        ...
        ComCall(CreateTexture2D := 5, ID3D11Device, "ptr", D3D11_TEXTURE2D_DESC, "ptr", 0, "ptr*", &staging_texture :=
            0)
    ...
    class DesktopDuplicationBuffer extends ImagePut.BitmapBuffer { ; Extending is optional!
        Update(timeout := unset) {
            ...
            loop {
                ...
                try ComCall(AcquireNextFrame := 8, this.IDXGIOutputDuplication, "uint", timeout ?? 0, "ptr",
                    DXGI_OUTDUPL_FRAME_INFO := Buffer(48), "ptr*", &desktop_resource := 0)
                catch OSError as e
...
FrameAcquired:
            ...
            if (this.DesktopImageInSystemMemory = 1) {
                ComCall(MapDesktopSurface := 12, this.IDXGIOutputDuplication, "ptr", DXGI_MAPPED_RECT := Buffer(
                    A_PtrSize * 2, 0))
                ...
                ComCall(_Map := 14, this.ID3D11DeviceContext, "ptr", this.staging_texture, "uint", 0, "uint",
                    D3D11_MAP_READ := 1, "uint", 0, "ptr", D3D11_MAPPED_SUBRESOURCE := Buffer(8 + A_PtrSize, 0))
    ...
    static HexToStream(image) {
        ...
        DllCall("crypt32\CryptStringToBinary", "str", image, "uint", 0, "uint", flags, "ptr", bin, "uint*", size, "ptr",
            0, "ptr", 0)
    ...
    static HBitmapToBitmap(image) {
        ...
        DllCall("GetDIBits", "ptr", hdc, "ptr", image, "uint", 0, "uint", 1, "uint*", &ARGB_1 := 0, "ptr", bi, "uint",
            0)
        if (ARGB_1 == 0xB0BA1234) {             ; Could be by pure coincidence
            NumPut("uint", 0xDECADEB4, pBits)    ; Test second pixel
            DllCall("GetDIBits", "ptr", hdc, "ptr", image, "uint", 0, "uint", 1, "uint*", &ARGB_2 := 0, "ptr", bi,
                "uint", 0)
    ...
    static RandomAccessStreamToStream(image) {
        ...
        DllCall("ole32\IIDFromString", "wstr", "{0000000C-0000-0000-C000-000000000046}", "ptr", IID_IStream := Buffer(
            16), "hresult")
        DllCall("shcore\CreateStreamOverRandomAccessStream", "ptr", image, "ptr", IID_IStream, "ptr*", &IStream := 0,
            "hresult")
    ...
    static WICBitmapToBitmap(image) {
        ...
        DllCall("ole32\CLSIDFromString", "wstr", "{6fddc324-4e03-4bfe-b185-3d77768dc90f}", "ptr",
            GUID_WICPixelFormat32bppBGRA := Buffer(16), "hresult")
        ...
        if (convert) {
            ...
            DllCall("windowscodecs\WICConvertBitmapSource", "ptr", GUID_WICPixelFormat32bppBGRA, "ptr", IWICBitmap,
                "ptr*", &IWICBitmapSource := 0, "hresult")
    ...
    static BitmapToClipboard(pBitmap) {
        ...
        DllCall("ole32\CLSIDFromString", "wstr", "{557CF406-1A04-11D3-9A73-0000F81EF32E}", "ptr", pCodec := Buffer(16),
        "hresult")
    ...
    static BitmapToSharedBuffer(pBitmap, name := "") {
        ...
        size := stride * height
        capacity := size + 12
        hMap := DllCall("CreateFileMapping", "ptr", -1, "ptr", 0, "uint", 0x4, "uint", 0, "uint", capacity, "str",
            "ImagePut_" name, "ptr")
        pMap := DllCall("MapViewOfFile", "ptr", hMap, "uint", 0x2, "uint", 0, "uint", 0, "uptr", 0, "ptr")
    ...
    class BitmapBuffer {
        ...
        pBitmap { ; Making .pBitmap dynamic ensures that wrapping the pointer is separate from GDI+ access.
            get { ; Test if .ptr property has changed due to an external API which returns a dynamic buffer.
                ...
                if this.HasProp("pBitmap2") {
                    try renew := DllCall("gdiplus\GdipGetImageType", "ptr", this.pBitmap2, "ptr*", &_type := 0) or (
                        _type != 1)
                    catch
                        renew := True
        ...
        Crop(x, y, w, h) {
            ...
            DllCall("gdiplus\GdipCloneBitmapAreaI", "int", x, "int", y, "int", w, "int", h, "int", format, "ptr", this.pBitmap,
                "ptr*", &pBitmap := 0)
            try return ImagePut.BitmapToBuffer(pBitmap)
            finally DllCall("gdiplus\GdipDisposeImage", "ptr", pBitmap)
        }

        Show(window_border := False, title := "", pos := "", style := "", styleEx := "", parent := "", playback := "",
            cache := "") {
        ...
        CPUID() {
            ...
            if not cpuid {
                ...
                n64 := StrLen(RTrim(b64, "=")) * 3 // 4
                code := DllCall("GlobalAlloc", "uint", 0, "uptr", n64, "ptr")
                DllCall("crypt32\CryptStringToBinary", "str", b64, "uint", 0, "uint", 0x1, "ptr", code, "uint*", n64,
                    "ptr", 0, "ptr", 0)
                DllCall("VirtualProtect", "ptr", code, "uptr", n64, "uint", 0x40, "uint*", 0)
...
redo: ; Unfortunately, unalignment can occur if the stride is not divisible by 4.
            ...
            if (option == 5) {
                ...
                address := DllCall(pixelsearch3, "ptr", ptr, "ptr", this.ptr + this.size, "ptr", colors, "uint", color.length,
                    "cdecl ptr")
            }
            ...
            if (option == 6) {
                ...
                address := DllCall(pixelsearch4, "ptr", ptr, "ptr", this.ptr + this.size, "ptr", high, "ptr", low,
                    "uint", color.length, "cdecl ptr")
            }
...
redo:
            ...
            ; When doing pointer arithmetic, *Scan0 + 1 is actually adding 4 bytes.
            if (option == 1)
                count := DllCall(pixelsearchall1, "ptr", result, "uint", limit, "ptr", this.ptr, "ptr", this.ptr + this
                    .size, "uint", color, "cdecl uint")
            ...
            if (option == 2) {
                ...
                count := DllCall(pixelsearchall2, "ptr", result, "uint", limit, "ptr", this.ptr, "ptr", this.ptr + this
                    .size
                    , "uchar", min(r + v, 255)
                    , "uchar", max(r - v, 0)
                    , "uchar", min(g + v, 255)
            ...
            if (option == 5) {
                ...
                count := DllCall(pixelsearchall3, "ptr", result, "uint", limit, "ptr", this.ptr, "ptr", this.ptr + this
                    .size, "ptr", colors, "uint", color.length, "cdecl ptr")
            }
            ...
            if (option == 6) {
                ...
                count := DllCall(pixelsearchall4, "ptr", result, "uint", limit, "ptr", this.ptr, "ptr", this.ptr + this
                    .size, "ptr", high, "ptr", low, "uint", color.length, "cdecl ptr")
            }
...
redo:
    ...
    static BitmapToWindow(pBitmap, title := "", pos := "", style := "", styleEx := "", parent := "", playback := "",
        cache := "") {
    ...
    static Show(pBitmap, title := "", pos := "", style := "", styleEx := "", parent := "", playback := "", cache := "") {
        ...
        obj := { scale: 5, scales: [0.125, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 3, 4, 6, 8, 12] }
        ...
        DllCall("gdiplus\GdipImageGetFrameDimensionsList", "ptr", pBitmap, "ptr", dimIDs := Buffer(16 * dims), "uint",
        dims)
        ...
        if (number > 1 && nDelays > 0) {
            ...
            ; Case 2: Image is scaled.
                ...
                loop number {
                    ...
                    NumPut("ushort", 1, bi, 12) ; Planes
                    NumPut("ushort", 32, bi, 14) ; BitCount / BitsPerPixel
                    hbm := DllCall("CreateDIBSection", "ptr", hdc, "ptr", bi, "uint", 0, "ptr*", &pBits := 0, "ptr", 0,
                        "uint", 0, "ptr")
                    obm := DllCall("SelectObject", "ptr", hdc, "ptr", hbm, "ptr")
    ...
    static WindowProc(hwnd, uMsg, wParam, lParam) {
        ...
        if (uMsg = 0x201)
            return DllCall("DefWindowProc", "ptr", obj.scales[obj.scale] > 1 ? child : parent, "uint", 0xA1, "uptr", 2,
                "ptr", 0, "ptr")
        ...
        if (uMsg = 0x207) {
            ...
            text_color := (0.3 * (255 & c >> 16) + 0.59 * (255 & c >> 8) + 0.11 * (255 & c)) >= 128 ? 0x000000 :
                0xFFFFFF
...
default:
    ...
    static SyncWindowProc(hwnd, uMsg, wParam := 0, lParam := 0) {
        ...
        if (A_PtrSize = 8) {
            ...
            DllCall("crypt32\CryptStringToBinary", "str", assembly, "uint", 0, "uint", 0x4, "ptr", pcb, "uint*", ncb,
                "ptr", 0, "ptr", 0)
    ...
    static BitmapToDesktop(pBitmap, title := "", pos := "", style := "", styleEx := "", parent := "", playback := "",
        cache := "") {
    ...
    static BitmapToCursor(pBitmap, xHotspot := "", yHotspot := "") {
        ...
        loop parse,
            "32512,32513,32514,32515,32516,32631,32642,32643,32644,32645,32646,32648,32649,32650,32651,32671,32672",
            ","
    ...
    static StreamToRandomAccessStream(stream) {
        ...
        DllCall("ole32\IIDFromString", "wstr", "{905A0FE1-BC53-11DF-8C49-001E4FC686DA}", "ptr", IID_IRandomAccessStream :=
            Buffer(16), "hresult")
        DllCall("shcore\CreateRandomAccessStreamOverStream", "ptr", stream, "uint", 1, "ptr", IID_IRandomAccessStream,
            "ptr*", &IRandomAccessStream := 0, "hresult")
        return IRandomAccessStream
    }
    ...
    static BitmapToWICBitmap(pBitmap) {
        ...
        IWICImagingFactory := ComObject("{CACAF262-9370-4615-A13B-9F5539DA4C0A}",
            "{EC5EC8A9-C395-4314-9C77-54D7A935FF70}")

        ; Initialize bitmap with backing memory. WICBitmapCacheOnDemand = 1
        DllCall("ole32\CLSIDFromString", "wstr", "{6fddc324-4e03-4bfe-b185-3d77768dc90f}", "ptr",
            GUID_WICPixelFormat32bppBGRA := Buffer(16), "hresult")
        ComCall(CreateBitmap := 17, IWICImagingFactory, "uint", width, "uint", height, "ptr",
            GUID_WICPixelFormat32bppBGRA, "int", 1, "ptr*", &IWICBitmap := 0)
    ...
    static RenderPDF(&IStreamIn, index := "") {
        ...
        DllCall("ole32\IIDFromString", "wstr", "{905A0FE1-BC53-11DF-8C49-001E4FC686DA}", "ptr", IID_IRandomAccessStream :=
            Buffer(16), "hresult")
        DllCall("shcore\CreateRandomAccessStreamOverStream", "ptr", IStreamIn, "uint", 1, "ptr",
            IID_IRandomAccessStream, "ptr*", &IRandomAccessStreamIn := 0, "hresult")

        ; Create the "Windows.Data.Pdf.PdfDocument" class using IPdfDocumentStatics.
        DllCall("ole32\IIDFromString", "wstr", "{433A0B5F-C007-4788-90F2-08143D922599}", "ptr", IID_IPdfDocumentStatics :=
            Buffer(16), "hresult")
        DllCall("combase\WindowsCreateString", "wstr", "Windows.Data.Pdf.PdfDocument", "uint", 28, "ptr*", &hString :=
            0, "hresult")
        DllCall("combase\RoGetActivationFactory", "ptr", hString, "ptr", IID_IPdfDocumentStatics, "ptr*", &
            PdfDocumentStatics := 0, "hresult")
        ...
        DllCall("shcore\CreateRandomAccessStreamOverStream", "ptr", IStreamOut, "uint", BSOS_DEFAULT := 0, "ptr",
            IID_IRandomAccessStream, "ptr*", &IRandomAccessStreamOut := 0)
    ...
    static RenderSVG(&IStream, width, height) {
        ...
        ; Initialize bitmap with backing memory. WICBitmapCacheOnDemand = 1
        DllCall("ole32\CLSIDFromString", "wstr", "{6fddc324-4e03-4bfe-b185-3d77768dc910}", "ptr",
            GUID_WICPixelFormat32bppPBGRA := Buffer(16), "hresult")
        ComCall(CreateBitmap := 17, IWICImagingFactory, "uint", width, "uint", height, "ptr",
            GUID_WICPixelFormat32bppPBGRA, "int", 1, "ptr*", &IWICBitmap := 0)
        ...
        DllCall("ole32\IIDFromString", "wstr", "{06152247-6f50-465a-9245-118bfd3b6007}", "ptr", IID_ID2D1Factory :=
            Buffer(16), "hresult")
        ...
        ComCall(CreateWicBitmapRenderTarget := 13, ID2D1Factory, "ptr", IWICBitmap, "ptr",
            D2D1_RENDER_TARGET_PROPERTIES, "ptr*", &ID2D1RenderTarget := 0)
        ...
        ComCall(CreateSvgDocument := 115, ID2D1RenderTarget, "ptr", IStream, "uint64", NumGet(D2D1_SIZE_F, "uint64"),
        "ptr*", &ID2D1SvgDocument := 0) ;ID2D1DeviceContext5::
    ...
    static select_codec(pBitmap, extension, quality, &pCodec, &ep) {
        ...
        switch extension, "Off" {
            ...
            case "bmp": DllCall("ole32\CLSIDFromString", "wstr", "{557CF400-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
            case "gif": DllCall("ole32\CLSIDFromString", "wstr", "{557CF402-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
            case "heic": DllCall("ole32\CLSIDFromString", "wstr", "{557CF408-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
            case "jpeg": DllCall("ole32\CLSIDFromString", "wstr", "{557CF401-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
            case "png": DllCall("ole32\CLSIDFromString", "wstr", "{557CF406-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
            case "tiff": DllCall("ole32\CLSIDFromString", "wstr", "{557CF405-1A04-11D3-9A73-0000F81EF32E}", "ptr",
                pCodec, "hresult")
        }
        ...
        if (extension = "jpeg") && (quality ~= "^\d+$") {
            ...
            DllCall("ole32\CLSIDFromString", "wstr", "{1D5BE4B5-FA4A-452D-9CDD-5DB35105E7EB}", "ptr", ep.ptr +
                A_PtrSize, "hresult")
    ...
    static gdiplus(vary := 0, cotype := "") {
        ...
        if (instances = 1 && vary = -1) {
            ...
            if (cotype = "bitmap") {
                ...
                if not assert
                    throw Error(
                        "Bitmap is out of scope. `n`nIf you wish to handle raw pointers to GDI+ bitmaps, add the line"
    ...
    class Destroy extends ImagePut {
        ...
        static Destroy(type, image) {
            switch type, "Off" {
                ...
                case "Wallpaper":
                    DllCall("SystemParametersInfo", "uint", SPI_SETDESKWALLPAPER := 0x14, "uint", 0, "ptr", 0, "uint",
                        2)
